      ...
      <div className="screen-content-split">
        ...
        <div className="chart-side glass-panel">
          ...
          <div className="macro-progress-summary">
            ...
            {/* Recent Logged Meals List (Bug #5 Fix) */}
            <div className="recent-meals-card glass-card" style={{ padding: '1.25rem' }}>
              <h4 style={{ fontSize: '0.9rem', marginBottom: '0.75rem', color: 'white', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <Utensils size={16} color="var(--color-safe)" /> Today's Logged Food Items
              </h4>
              {todayMeals.length === 0 ? (
                <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>No meals logged yet today.</p>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  {todayMeals.map((meal, idx) => (
                    <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(255,255,255,0.03)', padding: '0.6rem 0.8rem', borderRadius: '6px', border: '1px solid rgba(255,255,255,0.05)' }}>
                      <div>
                        <strong style={{ fontSize: '0.85rem', color: 'white' }}>{meal.foodItem || 'Logged Meal'}</strong>
                        <span style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', display: 'block' }}>Category: {meal.mealCategory || 'General'}</span>
                      </div>
                      <div style={{ fontSize: '0.8rem', color: 'var(--color-safe)', fontWeight: '600' }}>
                        {meal.calories ? `${meal.calories} kcal` : `${meal.volume || 250} ml`}
                      </div>
                    </div>
                  ))}
                </div>
