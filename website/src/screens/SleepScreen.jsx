...
export default function SleepScreen() {
  ...
  const points = recentSleepLogs.map((log, index) => {
    const totalCount = recentSleepLogs.length;
    const x = totalCount <= 1 
      ? chartWidth / 2 
      : padding + (index / (totalCount - 1)) * (chartWidth - padding * 2);
    const y = chartHeight - padding - ((log.duration || 0) / (maxHours || 1)) * (chartHeight - padding * 2);
    const logDate = log.endTime ? new Date(log.endTime) : new Date();
    const dateStr = isNaN(logDate.getTime()) ? 'Log' : logDate.toLocaleDateString([], { weekday: 'short' });
    return { x, y, duration: log.duration || 0, date: dateStr };
  });
