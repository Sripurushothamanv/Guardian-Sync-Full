    ...
    <div className="weekly-report-wrapper">
      ...
      <div className="screen-content-split" style={{ marginTop: '1rem' }}>
        ...
        <div className="chart-side glass-panel">
          ...
          <div className="weekly-report-chart-box">
            {/* Draw simplified SVG representing fatigue values */}
            <svg viewBox="0 0 320 120" className="svg-chart">
              {report.fatigueTrend.map((t, idx) => {
                const x = 30 + idx * 40;
                const h = (t.score / 100) * 80;
                const y = 100 - h;
                return (
                  <g key={idx}>
                    <rect x={x} y={y} width="16" height={h} rx="2" fill="var(--color-primary)" opacity="0.7" />
                    <text x={x + 8} y="112" fontSize="8" fill="var(--text-secondary)" textAnchor="middle">{t.day}</text>
                    <text x={x + 8} y={y - 4} fontSize="8" fontWeight="600" fill="white" textAnchor="middle">{t.score}</text>
                  </g>
                );
              })}
            </svg>
